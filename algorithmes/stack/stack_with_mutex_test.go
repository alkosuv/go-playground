...
// BenchmarkStackOperations Комплексаная оценка работы стуктуры
func BenchmarkStackWithMutexOperations(b *testing.B) {
...
func BenchmarkStackWithMutex_Push(b *testing.B) {
...
func BenchmarkStackWithMutex_Pop(b *testing.B) {
	b.ReportAllocs() // Включаем сбор метрик памяти
...
func BenchmarkStackWithMutex_Read(b *testing.B) {
	s := NewStack()
...
func TestStackWithMutex(t *testing.T) {
	s := NewStack()
